...
RAG (Retrieval-Augmented Generation) agent that indexes documents in a vector store (Milvus) and retrieves relevant
chunks to augment the LLM's answers with your own data. Built with LangGraph, LangChain, and Milvus Lite.
...
- `VECTOR_STORE_ID` - Identifier for the Milvus collection. If left empty, a new collection will be created when loading documents.
- `VECTOR_STORE_PATH` - Absolute path where the Milvus Lite database will be stored. For local development, use a path on your machine. In containers, this defaults to `/opt/app-root/src/data/vector_store`.
...
After loading, set the `VECTOR_STORE_ID` in your `.env` to the collection ID printed by the script.
...
1. **Document Indexing**: Documents are loaded from a text file, split into chunks, and embedded using the configured embedding model. The embeddings are stored in a Milvus Lite vector database.

2. **Query Processing**: When the user asks a question, the agent generates an embedding for the query and searches the vector store for the most relevant document chunks.
...
The agent uses LangGraph to orchestrate the retrieval and generation steps, and LangChain for the LLM and embedding model integrations.
