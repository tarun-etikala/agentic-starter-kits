"""Fixtures for LlamaIndex Websearch agent evals."""
...
@pytest.fixture
def agent_url() -> str:
    """LlamaIndex Websearch agent URL from env var or default localhost:8000."""
    return os.environ.get("LLAMAINDEX_WEBSEARCH_AGENT_URL", "http://localhost:8000")
...
SEARCH_EVIDENCE = ["redhat", "red hat"]
...
def known_tools() -> list[str]:
    """Tools available on the LlamaIndex Websearch agent."""
    return ["dummy_web_search"]


@pytest.fixture
def llamaindex_websearch_thresholds(eval_config: dict[str, Any]) -> dict[str, Any]:
    """Load the llamaindex_websearch section from the shared thresholds config."""
    return eval_config["llamaindex_websearch"]
...
def run_eval(
    ...
    tool_calls — LlamaIndex autolog captures tool spans automatically.
