"""Tool usage evals for the LlamaIndex Websearch agent.

Validates that the agent selects, calls, and uses tools correctly
for various query types. The LlamaIndex Websearch agent has a single
tool: dummy_web_search (FunctionTool) that returns canned results
containing "RedHat".

Tool calls are extracted from MLflow traces via MLflowTraceClient —
LlamaIndex autolog captures tool spans automatically. When MLflow is
configured, enrich_eval_result() pulls SpanType.TOOL spans into
TaskResult.tool_calls, enabling full F1 scorer coverage.
"""
