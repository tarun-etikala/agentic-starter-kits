"""Tool usage evals for the Google ADK agent.

Validates that the agent selects, calls, and uses tools correctly
for various query types. The ADK agent has a single tool: ``dummy_web_search``
that returns a canned answer about Red Hat OpenShift AI.
...
present (e.g. after MLflow tracing is enabled), we also verify tool
selection accuracy via scorers.
"""
