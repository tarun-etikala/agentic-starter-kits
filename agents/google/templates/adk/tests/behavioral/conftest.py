"""Fixtures for Google ADK agent evals."""
...
@pytest.fixture
def agent_url() -> str:
    """Google ADK agent URL from GOOGLE_ADK_AGENT_URL env var or default localhost:8000."""
    return os.environ.get("GOOGLE_ADK_AGENT_URL", "http://localhost:8000")
...
def known_tools() -> list[str]:
    """Tools available on the Google ADK agent."""
    return ["dummy_web_search"]


@pytest.fixture
def google_adk_thresholds(eval_config: dict[str, Any]) -> dict[str, Any]:
    """Load the google_adk section from the shared thresholds config."""
    return eval_config["google_adk"]
