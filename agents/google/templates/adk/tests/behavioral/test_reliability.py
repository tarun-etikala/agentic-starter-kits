"""Reliability (pass@k) evals for the Google ADK agent.
...
pytestmark = [pytest.mark.google_adk, pytest.mark.slow]
...
async def test_pass_at_k_tool_usage(
    ...
    """
    k = google_adk_thresholds.get("pass_at_k", 8)
    query = "What is Red Hat OpenShift AI?"
    expected_tools = ["dummy_web_search"]
    threshold = google_adk_thresholds.get("tool_selection_accuracy", 0.85)
...
async def test_pass_at_k_response_quality(
    ...
    threshold = google_adk_thresholds.get("response_coherence_accuracy", 0.75)
